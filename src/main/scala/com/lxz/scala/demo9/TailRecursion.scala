...
object TailRecursion {
  def main(args: Array[String]): Unit = {
    val logger = LoggerFactory.getLogger("TailRecursion")
    val list = List(1, 2, 3, 4);
    logger.info("result: {}", sum(list))
    logger.info("result: {}", tailrecSum(list))
    logger.info("result: {}", sum2(list))
    logger.info("result: {}", tailrecSum2(list))
  }
